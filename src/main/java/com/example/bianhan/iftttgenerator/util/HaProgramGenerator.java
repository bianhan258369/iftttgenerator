...
//        String requirementTexts = "light.loff";
        ...
        List<String> tempRules = new ArrayList<>();
        String rules = "";
        List<String> requirements = Arrays.asList(requirementTexts.split("//"));
        for(String requirement : requirements){
            if(requirement.contains(":")){
                String req = requirement.split(":")[1];
                if(req.indexOf(" ") == -1) commands.add(requirement);
                else tempRules.add(requirement);
            }
            else {
                if(requirement.indexOf(" ") == -1) commands.add(requirement);
                else tempRules.add(requirement);
            }
        }
        ...
        if(rules.length() > 0){
            ...
            List<String> reverseRequirements = computeReverseRequirements(rules, SMARTHOMEONTOLOGYPATH, 0);
            List<String> complementedRequirements = computeComplementedRequirements(SMARTHOMEONTOLOGYPATH);
            for(String requirement : reverseRequirements){
                rules = rules + "//" + requirement;
            }
            List<String> functionalRequirementsWithAreas = (List<String>) toFunctionalRequirements(rules, SMARTHOMEONTOLOGYPATH, 0, "").get("functionalRequirementsWithAreas");
            rules = "";
            for(String requirement : functionalRequirementsWithAreas){
                rules = rules + requirement + "//";
            }
            rules = rules.substring(0, rules.length() - 2);
...
//                        String currentState = json.getString("state");
            ...
            for (String entity_id : entity_id_mapping_to_domain_or_device_class.keySet()){
                ...
                String area_id = "";
                if(entity_id.contains("_m1s_")) area_id = "nonono";
                else area_id = "office";
    ...
    public static void runCommands(List<String> commands) throws IOException, InterruptedException {
        ...
        for(String command : commands){
            ...
            for(String tempRoom : rooms){
                ...
                System.out.println(deviceRegistryItemMap);
                DeviceRegistryItem deviceRegistryItem = deviceRegistryItemMap.get(state);
                String service = deviceRegistryItem.getService();
                message.put("service", service);
                JSONObject targetJSON = new JSONObject();
                for(Entity entity : entities){
                    if(entity.getDomainOrDeviceClass().equals(deviceRegistryItem.getDomain()) && entity.getArea().equals(tempRoom)){
